...
## [22:25] 持久化 SSH tunnel (systemd)
- **來源**: Happy Coder / CLI
- **Session**: c3708ab1
- **狀態**: ✅ 完成
- **目標**: 用 systemd 持久化 acmacmini2 到 ac-3090 的 SSH tunnel，開機自動啟動、斷線自動重連

### 實作內容
- 建立兩個 systemd service:
  - `ssh-tunnel-3090-compute.service` — port 9000 (Compute Plane API)
  - `ssh-tunnel-3090-vllm.service` — port 8000 (vLLM OpenAI-compatible API)
- 配置: `Restart=always`, `RestartSec=10`, `ServerAliveInterval=30`
- 已 enable（開機自動啟動）

### 驗證結果
- 兩個 service active (running) ✅
- tunnel restart 後自動恢復 ✅
- 高可用測試 16/16 通過 ✅
...
### 變更檔案
- acmacmini2: `/etc/systemd/system/ssh-tunnel-3090-compute.service` (新增)
- acmacmini2: `/etc/systemd/system/ssh-tunnel-3090-vllm.service` (新增)

## [21:00] 建立 SmallClawd 專案
- **來源**: Happy Coder / CLI
- **狀態**: ⏸️ 進行中（等待 GitHub 認證）
- **目標**: 建立輕量級個人 AI 助手平台，用 Subscribe 帳號在 RPi5 上運行
- **已完成**:
  - 建立專案目錄結構 ~/smallclawd/
  - 撰寫 OpenSpec 規格書（docs/SPEC.md）— 完整架構、雙層 LLM、30 個服務清單
  - 建立 README.md, .gitignore, .env.example
  - 初始 git commit (62ac060)
  - 清理 rpi5 workshop 容器（釋放 5GB 磁碟 + Docker images）
  - rpi5 現狀：磁碟 69% (8.6GB 可用), RAM 6.4GB 可用, Docker 0 容器
- **待完成**:
  - gh auth login（需手動操作）
  - 建立 GitHub repo 並 push
  - 用戶選擇 20 個服務（從 30 個預選中）
  - 註冊 SmallClawd GitHub 帳號（需手動）
  - USB SSD 掛載到 rpi5
- **變更檔案**:
...
- **恢復方式**: 執行 `gh auth login` 後，`cd ~/smallclawd && gh repo create AlanChen75/smallclawd --public --source . --push`

## [23:27] 自動化成本報告（Phase 5）
- **來源**: Happy Coder / CLI
- **Session**: c3708ab1
- **狀態**: ✅ 完成
- **目標**: 追蹤混合模式的 per-provider 使用量與成本，整合到每日報告

### 實作內容

**1. UsageTracker 類別（llm_router.py）**
- 新增 `UsageTracker` 類別，按日期 + provider 分類追蹤
- 記錄每個請求的 provider、model、成功/失敗、fallback、token 使用量
- OpenAI 成本估算（gpt-4.1-nano: $0.10/$0.40 per 1M tokens）
- vLLM 本地模型成本為 $0
- 所有路徑（generate/chat/tool_call + fallback chain）均記錄使用量

**2. usage-stats API 端點（proxy.py）**
- `GET /api/v1/admin/llm/usage-stats?date=YYYY-MM-DD`
- 回傳 per-provider 明細、token 數、成本估算
- 包含配置資訊和健康狀態

**3. 每日報告整合（daily-claude-report.py）**
- 新增 `fetch_shc_usage_stats()` 透過 localhost:8081 或 SSH 取得統計
- 新增 SHC LLM Router 段落，顯示 per-provider 明細
- 計算 vLLM 節省金額（對比全 OpenAI）
- SHC 成本合併到 Daily Total

### 測試結果
- 高可用測試 16/16 通過 ✅
- usage-stats API 正確追蹤所有請求 ✅
- 每日報告正確顯示 SHC 段落 ✅

### 變更檔案
- acmacmini2: `llm_router.py`（新增 UsageTracker 類別 + get_usage_stats()）
- acmacmini2: `proxy.py`（新增 /api/v1/admin/llm/usage-stats 端點）
- ac-mac: `/usr/local/bin/server-monitor/daily-claude-report.py`（新增 SHC 段落）
...
- Phase 2: 高可用 API 端點 ✅
- Phase 3: 智慧路由 + 真正混合模式 ✅
- Phase 4: Telegram 告警 + SSH Tunnel 持久化 ✅
- Phase 5: 自動化成本報告 ✅

## [23:45] 3090 內部 API 測試腳本設計完成
- **來源**: TG Bot
- **Session**: c44476a7
- **狀態**: ✅ 完成
- **目標**: 設計測試腳本，先執行 3090 內部 API 測試，查看 token 使用量與品質後再決定是否執行外部 API 測試
- **背景**:
  - RPi5 有 Tailscale ACL 限制，改用 acmacmini2 作為中繼
  - 需要先知道 3090 的 token 消耗量，才能評估外部 API 測試成本
  - 20 個任務涵蓋簡單、中等、複雜三類
- **完成項目**:
  - 測試腳本: `test_internal_api_vllm.py` (20 任務，自動評分，完整 log)
  - 分析腳本: `analyze_results.py` (統計報告、成本估算)
  - 使用文件: `README-API-COMPARISON.md` (完整指南)
- **架構**:
  ```
  ac-mac (測試腳本)
    ↓ localhost:8081
  acmacmini2:8081 (SHC Proxy)
    ↓ SSH tunnel :8000
  ac-3090:8000 (vLLM Qwen2.5-7B)
  ```
- **測試任務**:
  - A 類（簡單, 5個）: 質數函數、Docker解釋、命名建議、Bash腳本、翻譯
  - B 類（中等, 7個）: REST API設計、FastAPI CRUD、時間複雜度、SQL JOIN、HTML+CSS、Schema設計、pytest
  - C 類（複雜, 8個）: LRU Cache、微服務架構、爬蟲、JWT中介層、瓶頸分析、BST、分散式快取、MapReduce
- **評分機制**:
  - 自動評分（啟發式）: 完成度、正確性、品質、深度 (0-10 each)
  - 總分 = 平均 × 10 (0-100)
  - **需人工審核**正確性部分
- **Log 格式**:
  - JSONL (每行一個任務)
  - 包含: task_id, prompt, response, tokens, latency, evaluation
  - 支援 jq 查詢
- **預期結果**:
  - 總 Token: ~34,500 (A類 4K + B類 10.5K + C類 20K)
  - 內部成本: $0.00
  - 外部成本估算: ~$0.01 (OpenAI gpt-4.1-nano)
  - 執行時間: ~15-30 分鐘
- **下一步**:
  1. 用戶執行 `python3 test_internal_api_vllm.py`
  2. 查看 `python3 analyze_results.py logs/internal-vllm-*.jsonl`
  3. 根據報告決定是否執行外部 API 測試
- **變更檔案**:
  - ~/super-happy-tests/test_internal_api_vllm.py (新增, ~430 行)
  - ~/super-happy-tests/analyze_results.py (新增, ~230 行)
  - ~/super-happy-tests/README-API-COMPARISON.md (新增)
  - ~/super-happy-tests/logs/ (目錄)
