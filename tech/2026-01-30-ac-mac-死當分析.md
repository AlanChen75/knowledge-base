...
| 項目 | 優先級 | 狀態 |
|------|--------|------|
| 其他設備心跳監控（ac-mac 主動輪巡） | 高 | ⬜ 待處理 |
| 檢查 snap 套件清單 / 考慮停用 snapd | 高 | ⬜ 待處理 |
| 檢查有線網路（enp2s0f0 NO-CARRIER） | 中 | ⬜ 待處理 |
| Tailscale ACL 變更 SOP | 中 | ⬜ 待處理 |
| kernel watchdog 自動重啟設定 | 中 | ⬜ 待處理 |
| 限制 snapd CPU 使用（cgroup） | 低 | ⬜ 待處理 |
| 評估完全移除 snapd | 低 | ⬜ 待處理 |

#### 其他設備心跳監控方案筆記

**建議方案：ac-mac 主動輪巡（方案 A）**

在 `network_health_monitor` 迴圈中，每 5 分鐘 SSH ping 其他 3 台機器（3090、rpi5、macmini2），連續 3 次不通則發 Telegram 告警。

優點：
- 已有 SSH 連線基礎（`check_host_alive()` 現成）
- 集中管理，不需改動其他機器
- 告警通路統一走 Telegram

注意：ac-mac 自身掛掉的情況已由現有健康監控處理（自動重啟 tailscaled → 自動 reboot）
