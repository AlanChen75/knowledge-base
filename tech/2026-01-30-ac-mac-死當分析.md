...
### 6.1 短期（立即）
1. **Tailscale 連線監控腳本**：偵測 Tailscale 連線異常超過 N 分鐘，自動重啟 `tailscaled` 並發 Telegram 告警
   ```bash
   # 監控邏輯概念
   # 1. 定期檢查 tailscale status
   # 2. 如果連續 3 次失敗（間隔 1 分鐘），自動 systemctl restart tailscaled
   # 3. 發送 Telegram 通知
   ```
2. **檢查 snap 套件清單**：`snap list` 確認裝了哪些 snap
3. **考慮停用 snapd**：snapd 在網路異常時容易卡住並消耗大量 CPU，如果沒有必要的 snap 套件，建議停用
   ```bash
   sudo systemctl stop snapd
   sudo systemctl disable snapd
   sudo systemctl mask snapd
   ```
4. **檢查有線網路**：確認網線是否鬆脫或故障

### 6.2 中期
1. **Tailscale ACL 變更 SOP**：修改 ACL 前先確認受影響節點，變更後立即驗證各節點連線狀態
2. **設定自動重啟**：配置 kernel watchdog，系統凍結時自動重啟
   ```bash
   ...
   ```
3. **限制 snapd CPU 使用**：如果保留 snapd，設定 cgroup 限制其 CPU
4. **增加系統監控告警**：在 tg-monitor-bot 增加 CPU 使用率、Tailscale 連線狀態告警

### 6.3 長期
1. **評估是否完全移除 snapd**：Ubuntu Server 可以不依賴 snap，移除後可避免類似問題
2. **定期檢查系統日誌**：`journalctl -p err -b` 看有無異常
3. **建立完整的網路監控**：不只監控 Tailscale，也監控實體網路介面的 carrier 狀態
