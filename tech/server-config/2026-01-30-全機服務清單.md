...
POST /v1/ocr/submit             # OCR 提交（非同步，Surya OCR）
...
| 進程 | 佔用 |
|------|------|
| vLLM (Qwen2.5-7B FP16, 0.75) | ~18.5 GB |
| Surya OCR（延遲載入） | ~1.5 GB |
...
| OCR | ✅ | Surya OCR 0.17, 信心度 0.990, 中英混合正確 |

---

## 七、已知問題

1. ~~**OCR use_gpu 參數**~~：已修復，改用 Surya OCR 0.17（PyTorch 原生，取代 PaddleOCR）
...
4. **vLLM gpu-memory-utilization 調降**：從 0.85 降至 0.75，以預留 VRAM 給 Surya OCR (~1.5GB)
