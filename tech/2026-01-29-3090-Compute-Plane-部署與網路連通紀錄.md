...
- [x] 選擇並下載 LLM 模型 → Qwen2.5-7B-Instruct (15GB)
- [x] 啟動 vLLM server → systemd 服務 `vllm.service`，使用 FLASH_ATTN 後端
- [x] Compute Plane → vLLM 串接驗證（7/8 通過，OCR 待修）
- [ ] 將 Compute Plane API 整合到 Super Happy Coder proxy.py 的 Agent Executor
- [ ] TG Bot Token 取得後整合
- [ ] 解決 Tailscale iptables 不同步問題（長期）
- [ ] 修復 OCR `use_gpu` 參數不相容問題
