...
本知識庫匯集 61 篇筆記，涵蓋四條主線：(1) 模型選型、部署與量化（vLLM、QwenASR Int8、PaddleOCR-VL）；(2) Prompt Engineering 與成本控制（Token 優化、CLAUDE.md 精簡、模型降級策略）；(3) LLM 在專業領域的落地實踐（法律、會計、行銷、室內設計、學術研究）；(4) AI 對產業與社會結構的衝擊（企業 AI 五等級、教育系統重塑、職業角色轉型）。2026 年 4 月下旬再增兩篇重要筆記：AI 應用架構七層 Stack 分析揭示 Anthropic 正在主導 L1–L5 標準制定；Anthropic Project Deal 以 69 人議價市集實驗首次量化模型品質差距（Opus 4.5 vs Haiku 4.5）的現實影響；Meeting Capture Studio SDD 則展示以 Chrome Extension + Cloudflare Workers AI + Gemini 組合實現零成本會議知識萃取的完整技術棧。這些筆記共同描繪出一幅「從模型到應用到社會」的完整技術地圖。
...
### 11. AI 應用架構七層 Stack 與 Anthropic 標準化

2026 年 AI 應用架構正在形成可觀察的分層結構（OSI-style 七層：L1 模型 → L7 應用），最關鍵的洞察是 L1–L5 的核心標準幾乎全由 Anthropic 開放規範主導——MCP（L2 協議）、Claude Agent SDK 五大 workflow 命名（L3 框架）、SKILL.md（L4 Skills 能力）、CLAUDE.md/AGENTS.md（L5 Agent 身份）——業界正在大規模跟進，Google Cloud Next 2026 公開承認採用 Anthropic 的 SKILL.md 格式 [[2026-04-25-AI七層架構與Anthropic主導趨勢]]。ECC（Everything Claude Code，100K stars）橫跨 L3–L5 三層，內建 38 agents、156 skills、72 commands，並跨 Claude Code/Cursor/Codex/OpenCode/Gemini 五工具相容 [[2026-04-25-AI七層架構與Anthropic主導趨勢]]。harness engineering 的核心信念是：harness 每個元件都隱含「對模型不能做什麼」的假設，**模型變強，編排應該變薄**，這是 Anthropic 提出的架構哲學。

### 12. 模型品質差距（Agent Quality Gap）與公平性

Anthropic Project Deal 實驗（69 人、186 筆交易、$4,000 總值）首次在受控環境中量測：由 Opus 4.5 代表的使用者議價成果客觀上優於由 Haiku 4.5 代表者，但弱模型使用者的感知滿意度毫無差異——即「隱形不平等」。這個現象從模型比較的技術問題，升級為代理人服務的倫理問題：使用者無法識別自己所用模型的相對優劣，因此無法做出有意義的選擇 [[2026-04-26-Anthropic-Project-Deal-代理人市集實驗]]。

此實驗亦驗證了多代理議價作為 LLM 研究場景的可行性：用 Slack 作為市集基礎設施，讓 Agent 全程自主互動（人類完全不介入），提供了測量 LLM 能力差異在真實世界情境中的新方法論。對 LLM 評估社群而言，「Agent 在現實任務中的相對結果」比純 benchmark 分數更具外部效度 [[2026-04-26-Anthropic-Project-Deal-代理人市集實驗]]。

### 13. 會議知識萃取：LLM 驅動的自學習術語引擎

Meeting Capture Studio SDD 展示了以瀏覽器原生技術（Chrome Extension 抓 Google Meet 字幕，免 STT 成本）驅動 LLM 知識萃取管線：Workers AI（Llama/Gemini）為主、TF-IDF/jieba（JS）為輔進行術語提取，並設計四層信心分層校正引擎（L0 觀察 → L1 候選 → L2 正式 → L3 核心），術語可自動晉升/降級 [[2026-04-26-Meeting-Capture-Studio-SDD-規格書]]。

技術亮點是「自學習迴路」——每次會議的術語校正結果回饋到模型的上下文，逐步個人化術語表而無需微調。儲存架構採 D1（主）+ Notion + GitHub 雙鏡像，月成本 $0，並以每天定時批次 + 手動觸發並存，達到「完全自動、隨時可介入」的生產運維平衡 [[2026-04-26-Meeting-Capture-Studio-SDD-規格書]]。
...
> **Anthropic 已成為 Agent 層的 W3C**：MCP、SKILL.md、AGENTS.md、CLAUDE.md 四個開放規範分別主導協議、能力、身份三層。歷史類比：Web 標準（W3C）→ Cloud（AWS）→ Container（Google→CNCF）→ Agent（2024–2026, Anthropic）。Google 公開承認採用 Anthropic 格式，這在大廠生態中是少見的事。 [[2026-04-25-AI七層架構與Anthropic主導趨勢]]

> **模型品質差距在真實世界任務中已可量測**：Project Deal 的 186 筆議價交易證明，Opus 4.5 vs Haiku 4.5 的能力差異在複雜談判場景下轉化為可測量的成果差距，且使用者完全無感知——benchmark 分數與現實任務的差距首次在同一組使用者身上被雙盲量化。 [[2026-04-26-Anthropic-Project-Deal-代理人市集實驗]]

> **零成本會議知識萃取管線已可落地**：Chrome Extension 抓字幕（免 STT 費用）+ Workers AI 術語提取 + 四層信心分層 + D1 儲存，月成本 $0，且設計了自學習迴路讓術語表隨使用個人化。MVP 可在 8 小時課程內由學員完成，這是 LLM 知識萃取工具的「最低可行成本」新基準。 [[2026-04-26-Meeting-Capture-Studio-SDD-規格書]]
...
LLM Knowledge Base（Karpathy 式）讓結構從內容自動湧現，SecondBrain → Notion 管線以人工結構為骨架 [[knowledge-management-comparison]]。本 Wiki 本身就是前者的實踐——59 篇原始筆記經 LLM 編譯後產出結構化知識。Open NotebookLM 則是第三條路：支援多 AI 提供商的 RAG 檢索 + Podcast 生成 [[2026-01-25-open-notebookllm]]。

### AI 架構標準化的歷史軌跡

Claude Code 源碼洩漏 [[2026-03-31_Claude-Code源碼洩漏分析]] 揭示了 Anthropic 的內部架構設計，與 AI 七層 Stack 分析 [[2026-04-25-AI七層架構與Anthropic主導趨勢]] 形成一組「由內而外」的視角：前者看 Anthropic 如何設計自己的系統，後者看 Anthropic 的標準如何被外部採用。ECC（Everything Claude Code）橫跨 L3–L5 是二者交叉點——既是 Claude Code 生態系最大的開源實作，也是 SKILL.md 標準落地的具體案例。Skill 系統的跨領域驗證（律師、會計、室內設計）[[2026-03-01-AI-skill-時代來臨-律師實務應用案例]] [[2026-04-01_Claude-Code會計三表案例分析]] 則從 L4 Skills 層往上，證明七層架構 L4–L7 的應用段確實存在爆發式的垂直分化。
...
8. **七層 Stack 的 L6–L7 應用層競爭格局**：AI 七層架構筆記 [[2026-04-25-AI七層架構與Anthropic主導趨勢]] 指出 L6（平台，如 Cursor、Devin、Copilot）與 L7（垂直應用，如 DesignClaw、Hermes）正是個人和企業能切入的層次，但筆記尚未系統整理各 L7 垂直應用的競爭框架——哪些賽道已擠滿、哪些仍是藍海。

9. **harness engineering 的「編排應該變薄」假設的實驗驗證**：Anthropic 的 harness engineering 信念是「模型變強，編排應變薄」，但在本知識庫現有的 SHC v4/v5 混合架構 [[2026-01-31-shc-v4-hybrid-architecture]] 中，編排層反而愈來愈厚。需要一個實驗來驗證：當底層模型能力提升一個世代，現有的 harness 中哪些層可以被刪除？
