...
本知識庫匯集 59 篇筆記，涵蓋四條主線：(1) 模型選型、部署與量化（vLLM、QwenASR Int8、PaddleOCR-VL）；(2) Prompt Engineering 與成本控制（Token 優化、CLAUDE.md 精簡、模型降級策略）；(3) LLM 在專業領域的落地實踐（法律、會計、行銷、室內設計、學術研究）；(4) AI 對產業與社會結構的衝擊（企業 AI 五等級、教育系統重塑、職業角色轉型）。2026 年 4 月更新加入 AI 應用架構七層 Stack 分析，揭示 Anthropic 正在主導 L1–L5 的業界標準制定，從協議到框架到身份層全面佈局。這些筆記共同描繪出一幅「從模型到應用到社會」的完整技術地圖。
