...
### 自架個人數據主權服務：Dawarich 位置歷史

Dawarich 是開源自架位置歷史管理平台，Google Timeline 的替代方案（AGPL-3.0，Ruby on Rails 8.0 + PostGIS，GitHub Stars 8.7k）。核心功能：互動式地圖＋熱力圖、旅行統計（到訪國家/城市/移動距離）、家庭共享（雙方同意後互看）、Immich/PhotoPrism 照片位置整合、支援 Google Maps Timeline / GPX / GeoJSON / OwnTracks 等資料匯入。以 Docker Compose 部署，資源佔用低（256MB RAM、0.1 CPU），可與 Glance 儀表板整合監控。評分：4/5，缺點是需 Ruby + Redis 技術棧（相對 Python 生態較少人熟悉），且官方強調不應用於追蹤他人。它代表了「數位主權基礎設施」的新類別——不只是 GPU 和 Agent，個人數據（位置歷史、照片、健康數據）的自架管理也是完整 AI 基礎設施的一部分 [[2026-04-20_Dawarich自架位置歷史追蹤工具分析]]。
...
> **數位主權基礎設施不只是 GPU**：Dawarich 的案例說明，個人數據自主權（位置歷史、照片、健康紀錄）同樣是現代個人 AI 基礎設施的組成部分。Google Timeline 停服後，自架替代方案的需求迅速上升，256MB RAM 的低資源消耗使其幾乎不構成硬體門檻。——[[2026-04-20_Dawarich自架位置歷史追蹤工具分析]]
