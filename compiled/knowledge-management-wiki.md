...
目前的發展階段是「基礎管線已通，智能層待疊加」。SecondBrain 的收集→分類→同步已穩定運作，下一步是在此基礎上加入知識編譯（LLM 自動產 wiki）、圖譜導航（Graph-based retrieval）、以及 RAG 診斷（語義防火牆）等能力。寫作輸出作為知識管理的最終環節，也需要從「網→樹→線」的轉換框架來理解。[[2026-02-03-寫作之難從網到樹到線]]
...
**技術路線對比：Vector RAG vs Graph RAG**。[[2026-03-30_悠識RAG系統規劃]] 代表傳統 Vector RAG 路線（Qdrant + Embedding），而 [[2026-04-09_Graphify-RAG機制與對話知識庫方案深度分析]] 代表 Graph RAG 路線（NetworkX + Leiden）。兩者的取捨核心在於：Vector RAG 生態成熟但需持續維護向量庫，Graph RAG 概念新穎但尚未經過大規模生產驗證。
...
- **混合檢索架構**：結合 Vector RAG 的語意搜尋與 Graph RAG 的關聯導航，取兩者之長。例如先用向量搜尋定位候選節點，再用圖譜展開相關概念。
